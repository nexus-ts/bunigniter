...
export class Admin extends Controller {
  ...
  async create() {
    ...
    await this.db.insert('posts', { title: v.data.title, slug, content: this.body?.content || '', excerpt: this.body?.excerpt || '', user_id: user.id })
    return this.redirect('/admin')
  }
  async update(id: number) {
    await this.db.update('posts', { title: this.body?.title, slug: this.body?.slug, content: this.body?.content || '', excerpt: this.body?.excerpt || '' }, { id })
    return this.redirect('/admin')
  }
}
