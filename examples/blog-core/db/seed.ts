...
const base = join(import.meta.dirname, '..', '..', 'blog-app-html', 'data')
if (!existsSync(base)) mkdirSync(base, { recursive: true })
const db = new Database(join(base, 'blog.db'))
...
if ((db.query('SELECT count(*) as c FROM users').get() as any).c === 0) {
  ...
  const posts = [
    ['Getting Started with NexusTS','getting-started-nexusts','NexusTS is a Bun-native framework inspired by CodeIgniter. It combines Rendu templates, React SSR, and MDX.','A Bun-native framework for PHP devs.',1],
    ['Elysia v2 Multi-Runtime','elysia-v2-multi-runtime','Elysia v2 introduces the Adapter system for Bun, Node, Deno, and CF Workers.','Run anywhere with the new adapter API.',1],
    ['Rendu Templates Guide','rendu-templates-guide','PHP-style templates with <?= ?> syntax. No build step needed.','PHP-style templates without the fatigue.',1],
    ['MDX in NexusTS','mdx-in-nexusts','Markdown + embedded React components. Best of both worlds.','Markdown with superpowers.',1],
    ['Session Management','session-management','Cookie sessions with AES-256-GCM encryption.','Secure sessions without Redis.',1],
    ['OpenAPI Auto-Docs','openapi-auto-docs','GET /openapi.json and GET /docs (Scalar UI). Zero config.','API docs from your routes.',1],
  ]
...
} else console.log(`[seed] ${(db.query('SELECT count(*) as c FROM posts').get() as any).c} posts`)
