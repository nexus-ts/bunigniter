...
describe("rateLimiter", () => {
	it("allows requests under limit", async () => {
		const app = new Elysia().use(rateLimiter({ max: 10, window: 60000, skip: [] })).get("/test", () => "ok")
	...
	it("blocks requests over limit", async () => {
		const app = new Elysia().use(rateLimiter({ max: 2, window: 60000, skip: [] })).get("/test", () => "ok")
	...
	it("skips configured paths", async () => {
		const app = new Elysia().use(rateLimiter({ max: 0, skip: ["/health"] })).get("/health", () => "ok")
	...
	it("returns retry-after header when blocked", async () => {
		const app = new Elysia().use(rateLimiter({ max: 1, window: 60000, skip: [] })).get("/test", () => "ok")
