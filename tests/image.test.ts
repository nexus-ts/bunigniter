...
describe("Image", () => {
	...
	describe("modulate()", () => {
		it("adjusts brightness", async () => {
			...
			await Image.open(TEST_INPUT).modulate({ brightness: 20, saturation: -10 }).save(out)
			expect(existsSync(out)).toBe(true)
		})
	})
	...
	describe("watermark()", () => {
		it("adds image watermark", async () => {
			...
			await Image.open(TEST_INPUT).watermark(wmPath, "bottom-right", 0.5).save(out)
			expect(existsSync(out)).toBe(true)
		})

		it("throws on missing watermark file", () => {
			expect(() => Image.open(TEST_INPUT).watermark("missing.png")).toThrow("not found")
		})
	})
	...
	describe("chained operations", () => {
		it("chains resize + greyscale + save", async () => {
			...
			await Image.open(TEST_INPUT).resize(80, 80).greyscale().sharpen(1).save(out)
			expect(existsSync(out)).toBe(true)
		})
		...
		it("chains resize + watermark + save", async () => {
			...
			await Image.open(TEST_INPUT).resize(80).watermark(wmPath, "bottom-right", 0.5).save(out)
			expect(existsSync(out)).toBe(true)
		})
	})
