...
import { Upload, createUpload } from "../src/helpers/upload";
...
describe("Upload", () => {
	...
	describe("hasFile()", () => {
		...
		it("returns true with explicit body", () => {
			const upload = createTestUpload();
			expect(upload.hasFile(mockBody({ avatar: mockFile() }), "avatar")).toBe(
				true,
			);
		});
	...
	describe("files()", () => {
		it("returns multiple uploaded files", async () => {
			...
			upload.body = mockBody({
				gallery: [
					mockFile({ name: "img1.jpg", size: 100 }),
					mockFile({ name: "img2.jpg", size: 200 }),
				],
			});
	...
	describe("extension()", () => {
		it("returns lowercased extension", () => {
			const upload = createTestUpload();
			expect(
				upload.extension({ field: "", name: "Photo.JPG", size: 0, type: "" }),
			).toBe(".jpg");
		});
		...
		it("handles files without extension", () => {
			const upload = createTestUpload();
			expect(
				upload.extension({ field: "", name: "README", size: 0, type: "" }),
			).toBe("");
		});
	});
	...
	describe("store()", () => {
		...
		it("stores with custom filename", async () => {
			...
			expect(
				existsSync(join(process.cwd(), TEST_STORAGE, subdir, "profile.jpg")),
			).toBe(true);
		});
		...
		it("throws on duplicate file without overwrite", async () => {
			...
			await expect(upload.store(file!, subdir, "existing")).rejects.toThrow(
				"already exists",
			);
		});
	});
