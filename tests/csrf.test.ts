...
describe("csrfMiddleware", () => {
	...
	it("provides csrfToken in context", async () => {
		const app = new Elysia().use(csrfMiddleware()).get("/token", ({ csrfToken }: any) => {
	...
	it("provides csrf.validate() function", async () => {
		const app = new Elysia().use(csrfMiddleware()).get("/check", ({ csrf }: any) => {
	...
	it("rejects POST without valid token", async () => {
		const app = new Elysia().use(csrfMiddleware()).post("/submit", ({ csrf }: any) => {
			if (!csrf.validate()) {
		...
		const res = await app.handle(new Request("http://localhost/submit", { method: "POST" }))
	...
	it("accepts POST with valid token header", async () => {
		...
		let savedToken = ""
		const app = new Elysia().use(csrfMiddleware()).post("/submit", ({ csrf, csrfToken }: any) => {
	...
	it("excludes specified paths", async () => {
		const app = new Elysia().use(csrfMiddleware({ exclude: ["/webhook"] })).post("/webhook", ({ csrf }: any) => {
		...
		const res = await app.handle(new Request("http://localhost/webhook", { method: "POST" }))
